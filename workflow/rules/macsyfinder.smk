...
rule macsyfinder:
    ...
    shell:
        ...
        if [[ -s {input.fasta:q} ]] ; then
            macsyfinder --models-dir {params.macsydata:q} --sequence-db {input.fasta:q} -o {output.results:q} --db-type {params.db_type} --replicon-topology {params.replicon_topology} -w {threads} {params.cut_ga} --models {params.models} &> {log:q}
        else
            echo {input.fasta}
